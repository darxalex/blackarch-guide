...
\section{Overview}
La Guia de BlackArch Lnux está dividido principalmenete en en estas partes:
\begin{itemize}
\item Introducción - Proporciona una visión general, una Introducción e información util acerca del proyecto.
\item Guia de usuario - Todo lo que un usuario típico necesita saber para usar con eficacia BlackArch
\item Guia de desarrollo - Como comenzar a desarrolar para contribuir a BlackArch
\item Guia de herramientas -  Ejemplos detallados en profundidad sobre el uso de las herramientas (WIP)
\end{itemize}

\section{Que es BlackArch Linux?}
BlackArch es una completa distribución Linux para pruebas de penetración e investigadores de seguridad.
Esta derivada de \href{https://www.archlinux.org/}{ArchLinux} y los usuarios pueden instalar los componentes de BlackArch 
individualmente o en grupos directamente desde la parte superior.

El conjunto de herramientas se distribuyen como en Arch Linux
\href{https://wiki.archlinux.org/index.php/Unofficial\_User\_Repositories}
{unofficial user repository} tambien puede instalar BlackArch a partir de una instalación
de Arch Linux ya existente. Los paquetes pueden instalarse induvidualmente o por categorias.

El repositorio que cada dia se hace mas grande incluye más de \href{https://www.blackarch.org/tools.html}{1300} herramientas.
Todas las herramientas han sido probadas antes de añadirse al código base para mantener la calidad del repositorio.
...
\section{Involucrese}
Puede ponerse en contacto con el equipo BlackArch utilizando las siguientes vías:

Página web: \url{https://www.blackarch.org/}

Correo: \href{mailto:team@blackarch.org}{team@blackarch.org}
...
\chapter{Guia de usuario}

\section{Instalación}
La siguiente sección le mostrará cómo configurar el repositorio de BlackArch e
instalar paquetes. BlackArch soporta tanto la instalación desde el repositorio usando
paquetes binarios asi como la compilación e instalación desde las fuentes.

BlackArch es compatible con las instalaciones de Arch normales. Este actua como un
repositorio no oficial del usuario. Si usted quiere un ISO, revise la sección de
\href{https://www.blackarch.org/downloads.html#iso}{Descarga de ISOS}.

\subsection{Instalación desde arriba de ArchLinux}
Ejecute \href{https://blackarch.org/strap.sh}{strap.sh} Como root y siga las
instrucciones. Como en el siguiente ejemplo.
\begin{lstlisting}
...
Descargue de nueva una copia fresca de la lista con los paquetes principales y sincronicelos:
\begin{lstlisting}
  sudo pacman -Syyu
\end{lstlisting}
...
Ya puede instalar las herramientas desde el repositorio de BlackArch .
\begin{enumerate}
\item con la lista de todos los paquetes disponibles, ejecute
\begin{lstlisting}
...
\Para instalar las herramientas de una sola categoria, ejecute
\begin{lstlisting}
  pacman -S blackarch-<category>
\end{lstlisting}

\Para ver todas la categorias de BlackArch, ejecute
\begin{lstlisting}
...
\subsection{Instalación de paquetes desde una fuente}
Como parte de un método alternativo de instalación, usted puede construir BlackArch
desde los paquetes fuente. Ustede debe encontrar los PKGBUILDs en
\href{https://github.com/BlackArch/blackarch/tree/master/packages}{github}. Para construir 
el repositorio entero, puede usar
\href{https://github.com/BlackArch/blackman}{Blackman} tool.
\begin{itemize}
\item Primero, tiene que instalar Blackman. Si BlackArch el repositorio de paquetes
está configurado en su máquina, puedwes instalar Blackman:
\begin{lstlisting}
  pacman -S blackman
\end{lstlisting}

\item Usted puede construir e instalar Blackman desde el paquete fuente:
\begin{lstlisting}
  mkdir blackman
  cd blackman
  wget https://raw2.github.com/BlackArch/blackarch/master/packages/blackman/PKGBUILD
  # Para asegurarse que PKGBUILD no ha sido manipula maliciosamente.
  makepkg -s
\end{lstlisting}

\item O usted puede instalar Blackman desde el AUR:
\begin{lstlisting}
